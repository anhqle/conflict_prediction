...
Res <- foreach (i=(1:length(cEOIs))) %dopar% {
  ...
  # Train the model
  formula <- paste(eoi, "~ .")
  assign(paste0("m_", eoi), logit.spike(formula, data=get(eoi)[cTRAIN, ], niter=5000))
  cat(eoi, "training done \n")

  # Predict in of sample
  in_pred_prob <- apply(predict(get(paste0("m_", eoi)), newdata=get(eoi)[cTRAIN, ],
                        burn=1000, type="response", na.action=na.pass), 1, mean)
  ...
  list(in_performance=in_performance,
              out_performance=out_performance,
              model=get(paste0("m_", eoi)))
}